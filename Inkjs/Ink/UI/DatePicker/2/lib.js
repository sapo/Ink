/**
 * @module Ink.UI.DatePicker_2
 * @author inkdev AT sapo.pt
 * @version 1
 */
Ink.createModule('Ink.UI.DatePicker', '2', ['Ink.UI.Aux_1','Ink.Dom.Event_1','Ink.Dom.Css_1','Ink.Dom.Element_1','Ink.Dom.Selector_1','Ink.Util.Array_1','Ink.Util.Date_1'], function(Aux, Event, Css, Element, Selector, InkArray, InkDate ) {
    'use strict';    
     ...
     *      @param {Boolean}  [options.weekDayNameAbbr]  Number letters
     *      @param {Boolean}  [options.othermonths]      Complete calendar with other months days
     *      @param {Boolean}  [options.inline]           Show Calendar instance inline in document
     *      @param {String|DOMElement} [options.inline_container]   Container for calendar instance
     *
     * @example
    ...
    var DatePicker = function(selector, options) {
        ...
        this._options = Ink.extendObj({
            ...
            format:          'yyyy-mm-dd',
            cssClass:        'ink-datepicker-container',
            position:        'right',
            onFocus:         false,
    ...
    DatePicker.prototype = {
        ...
        _render: function() {
            ...
            if(this._options.showClose || this._options.showClean){
                ...
                this._superTopBar.className = 'ink-datepicker-options';
                if(this._options.showClean && !this._options.inline){
                ...
                if(this._options.showClose && !this._options.inline){
            ...
            calendarTop.className = 'ink-datepicker-header';

            this._monthDescContainer = document.createElement("div");
            this._monthDescContainer.className = 'ink-datepicker-month-desc';

            this._monthPrev = document.createElement('div');
            this._monthPrev.className = 'ink-datepicker-prev';
            this._monthPrev.innerHTML ='<a href="#prev" class="ink-datepicker-change-month-prev">' + this._options.prevLinkText + '</a>';

            this._monthNext = document.createElement('div');
            this._monthNext.className = 'ink-datepicker-next';
            this._monthNext.innerHTML ='<a href="#next" class="ink-datepicker-change-month-next">' + this._options.nextLinkText + '</a>';
            ...
            this._monthContainer = document.createElement("table");
            this._monthContainer.className = 'ink-datepicker-calendar';
            ...
            this._monthSelector.className = 'ink-datepicker-month-selector unstyled';

            for(var i=1; i<=12; i++){
                var liMonth = document.createElement('li');
                liMonth.innerHTML = '<a href="#" class="ink-datepicker-calmonth-' + ( (String(i).length === 2) ? i : "0" + i) + '">' + this._options.month[i].substring(0,3) + '</a>';
                this._monthSelector.appendChild(liMonth);
            }
            
            this._containerObject.appendChild(this._monthSelector);

            this._yearSelector = document.createElement('ul');
            this._yearSelector.className = 'ink-datepicker-year-selector unstyled';

            this._containerObject.appendChild(this._yearSelector);

            if(!this._options.onFocus || this._options.displayInSelect || !this._options.inline){
                if(!this._options.pickerField){
                    ...
                    this._picker.className = 'ink-datepicker-picker';
                } else {
                    this._picker = Aux.elOrSelector(this._options.pickerField, 'pickerField');
                }
            }
            ...
            if (this._options.inline && this._options.inline_container) {
                this._options.inline_container = Aux.elOrSelector(this._options.inline_container, 'inlineContainer');
                this._options.inline_container.appendChild(this._containerObject);
            } else {
            ...
            if (!this._picker) {
                if (this._options.inline) {
                    if (!this._options.inline_container)
                    {
                    ...
                    this._dataField.style.display = 'none';
                } else {
            ...
            this._monthChanger.className = 'ink-datepicker-link-month';
            ...
            this._yearChanger.className = 'ink-datepicker-link-year';
            ...
            Event.observe(this._containerObject,'click',Ink.bindEvent(function(e){
                    ...
                    var isInactive  = className.indexOf( 'ink-datepicker-off' ) !== -1;

                    Event.stop(e);

                    if( className.indexOf('ink-datepicker-link-') === 0 && !isInactive ){
                            var day = className.substr( 20 , 2 );
                            if( Number( day ) ) {
                                this.setDate( this._year + '-' + ( this._month + 1 ) + '-' + day );
                                if (!this._options.inline) {
                                    this._containerObject.style.display = 'none';
                                } else {
                                    InkArray.each(Ink.ss('.ink-datepicker-selected'), function(el) {
                                        Css.removeClassName(el, 'ink-datepicker-selected');
                                    });
                                    Css.addClassName(e.target.parentNode, 'ink-datepicker-selected');
                                }
                            } else if(className === 'ink-datepicker-link-month'){
                            ...
                            } else if(className === 'ink-datepicker-link-year'){
                                this._monthPrev.childNodes[0].className = 'action_inactive';
                                this._monthNext.childNodes[0].className = 'action_inactive';
                    ...
                    } else if( className.indexOf("ink-datepicker-calmonth-") === 0 && !isInactive ){
                            var month=className.substr(24,2);
                            if(Number(month)){
                                this._month = month - 1;
                                ...
                                // }
                                this._monthSelector.style.display = 'none';
                                this._monthPrev.childNodes[0].className = 'ink-datepicker-change-month-prev';
                                this._monthNext.childNodes[0].className = 'ink-datepicker-change-month-next';
                                ...
                                this._updateCal();
                                this._monthContainer.style.display = 'table';
                            }
                    } else if( className.indexOf("ink-datepicker-calyear-") === 0 && !isInactive ){
                            var year=className.substr(23,4);
                            if(Number(year)){
                                this._year = year;
                    ...
                    } else if( className.indexOf('ink-datepicker-change-month-') === 0 && !isInactive ){
                            if(className === 'ink-datepicker-change-month-next'){
                                this._updateCal(1);
                            } else if(className === 'ink-datepicker-change-month-prev'){
                                this._updateCal(-1);
                            }
                    } else if( className.indexOf('ink-datepicker-change-year-') === 0 && !isInactive ){
                            if(className === 'ink-datepicker-change-year-next'){
                                this._showYearSelector(1);
                            } else if(className === 'ink-datepicker-change-year-prev'){
                                this._showYearSelector(-1);
                            }
            ...
            for (var i=0; i<=11; i++){
                if (!i || i === 11){
                    if ( i && (ano_base+i-1)<=this._yearMax && (ano_base+i-1)>=this._yearMin ){
                        str+='<li><a href="#year_next" class="ink-datepicker-change-year-next">' + this._options.nextLinkText + '</a></li>';
                    } else if( (ano_base+i-1)<=this._yearMax && (ano_base+i-1)>=this._yearMin ){
                         str+='<li><a href="#year_prev" class="ink-datepicker-change-year-prev">' + this._options.prevLinkText + '</a></li>';
                    } else {
                        str +='<li>&nbsp;</li>';
                    }
                } else {
                    if ( (ano_base+i-1)<=this._yearMax && (ano_base+i-1)>=this._yearMin ){
                        str+='<li><a href="#" class="ink-datepicker-calyear-' + (ano_base+i-1)  + (((ano_base+i-1) === this._data.getFullYear()) ? ' ink-datepicker-on' : '') + '">' + (ano_base+i-1) +'</a></li>';
                    } else {
                        str+='<li><a href="#" class="ink-datepicker-off">' + (ano_base+i-1) +'</a></li>';
       ...
       _writeDateInFormat:function(){
        ...
        _setDate : function( objClicked ){
            if( typeof objClicked !== 'undefined' && objClicked.className && objClicked.className.indexOf('ink-datepicker-') === 0 )
            {
        ...
        _showMonth: function(){
            ...
            var html = '<thead><tr>';
            var weekDayNameAbbr = this._options.weekDayNameAbbr || 1;
            for(i=0; i<7; i++){
                var weekDay = (((startWeekDay+i)>6) ? 0 : startWeekDay+i);
                var cssClass = ((weekDay == 0) || (weekDay == 6)) ? 'ink-datepicker-weekend' : 'ink-datepicker-weekday';
                html+='<th class="'+cssClass+'">' + this._options.wDay[weekDay].substring(0,weekDayNameAbbr)  + '</th>';
            }
            html+='</tr></thead>';

            var otherMonthDay = "&nbsp;"; var counter = 0;
            for (var i = 1; i <= maxDay; i++, counter++) {
                if (counter === 7) {
                    html+='</tr>';
                    counter=0;
                }
                ...
                var weekDay = (((startWeekDay+counter)>6) ? 0 : startWeekDay+counter);
                var cssClass = ((weekDay == 0) || (weekDay == 6)) ? 'ink-datepicker-weekend' : 'ink-datepicker-weekday';
                if (i == 1 && wDayFirst) { // antes de 
                    for(j = startWeekDay; j < wDayFirst - startWeekDay; j++) {
                        if (this._options.othermonths) {
                            otherMonthDay = (new Date( ano , mes - 1 ,  -(wDayFirst - startWeekDay - j -1))).getDate();
                        }
                        weekDay = (((startWeekDay+counter)>6) ? startWeekDay-7 : startWeekDay);
                        cssClass = ((weekDay == 0) || (weekDay == 6)) ? 'ink-datepicker-weekend' : 'ink-datepicker-weekday';
                        html+='<td class="ink-datepicker-prev-month ink-datepicker-other-month '+cssClass+'">' + otherMonthDay + '</li>';
                        counter++;
                    }
                }

                var idx = 'ink-datepicker-link-' + ((String(i).length === 2) ? i : "0" + i);
                var disabled = ( ano === this._yearMin && mes === this._monthMin && i < this._dayMin ||
                    ...
                    ano < this._yearMin || ano > this._yearMax || ( this._options.validDayFn && !this._options.validDayFn.call( this, new Date( ano , mes - 1 , i) ) ) );
                idx += disabled ? " ink-datepicker-day-disabled" 
                                : (this._data.getFullYear( ) === ano && this._data.getMonth( ) === mes - 1 && i === this._data.getDate()) 
                                    ? " ink-datepicker-today" 
                                    : "";
                html+='<td class="' + idx + ' ' + cssClass + '">';
                // not put a if disabled
                html += disabled ? i : '<a href="#" class="' + idx + '">' + i + '</a>';
                html+='</td>';
            }
            if (counter !== 7){
                var otherMonthDay = '&nbsp;';
                for(var j=1; counter < 7; counter++, j++){
                    if (this._options.othermonths) {
                        otherMonthDay = j;
                    }
                    var weekDay = (((startWeekDay+counter)>6) ? 0 : startWeekDay+counter);
                    var cssClass = ((weekDay == 0) || (weekDay == 6)) ? 'ink-datepicker-weekend' : 'ink-datepicker-weekday';
                    html+='<td class="ink-datepicker-next-month ink-datepicker-other-month '+cssClass+'">' + otherMonthDay + '</td>';
                }
                html+='</tr>';
            }
            html+='</tbody>';
        ...
        _setActiveMonth: function(parent){
            ...
            if (parent.className && parent.className.match(/ink-datepicker-calmonth-/)) {
                var year = this._year;
                var month = parent.className.substr( 14 , 2 );

                if ( year === this._data.getFullYear( ) && month === this._data.getMonth( ) + 1 )
                {
                    Css.addClassName( parent , 'ink-datepicker-on' );
                    Css.removeClassName( parent , 'ink-datepicker-off' );
                }
                else
                {
                    Css.removeClassName( parent , 'ink-datepicker-on' );
                    if ( year === this._yearMin && month < this._monthMin ||
                        year === this._yearMax && month > this._monthMax ||
                        year < this._yearMin ||
                        year > this._yearMax )
                    {
                        Css.addClassName( parent , 'ink-datepicker-off' );
                    }
                    else
                    {
                        Css.removeClassName( parent , 'ink-datepicker-off' );
        ...
        isMonthRendered: function(){
            var header = Selector.select('.ink-datepicker-header',this._containerObject)[0];
