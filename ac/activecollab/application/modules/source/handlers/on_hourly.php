<?php
  /**
   * Source version control module on_hourly event handler
   *
  ...
  /**
   * Hourly update of repositories
   *
   * @param null
   * @return void
   */
  function source_handle_on_hourly() {
    
    require_once(ANGIE_PATH.'/classes/xml/xml2array.php');
    
    $results = 'Repositories updated: ';
    
    $repositories = Repositories::findByUpdateType(REPOSITORY_UPDATE_HOURLY);
    ...
    return is_foreachable($repositories) && count($repositories) > 0 ? $results : 'No repositories for hourly update';
