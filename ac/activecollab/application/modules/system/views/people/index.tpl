...
{if is_foreachable($companies)}
  {if $pagination->getLastPage() > 1}
    <p class="pagination top"><span class="inner_pagination">{lang}Page{/lang}: {pagination pager=$pagination}{assemble route=people page='-PAGE-'}{/pagination}</span></p>
    <div class="clear"></div>
  {/if}
  ...
  {if ($pagination->getLastPage() > 1) && !$pagination->isLast()}
    <p class="next_page"><a href="{assemble route=people page=$pagination->getNextPage()}">{lang}Next Page{/lang}</a></p>
  {/if}
{else}
...
{/if}
  ...
  <ul class="category_list">
    <li class="selected"><a href="{assemble route=people}"><span>{lang}Companies{/lang}</span></a></li>
  {if $logged_user->isPeopleManager()}
    <li><a href="{assemble route=people_archive}"><span>{lang}Archive{/lang}</span></a></li>
  {/if}
  </ul>
</div>