<?php
  ...
  /**
   * Comments unlocked activity log
   *
   * @package activeCollab.modules.resources
   * @subpackage models
   */
  class CommentsUnlockedActivityLog extends ActivityLog {
     ...
     */
    function getIconUrl() {
      return get_image_url('activity_log/unlocked.gif');
    } // getIconUrl
    ...
    function renderHead($object = null, $in_project = false) {
      ...
      if(instance_of($object, 'ProjectObject')) {
        ...
        if ($in_project) {
          return lang('<a href=":user_url">:user_name</a> unlocked comments on <a href=":url">:name</a> :type', $lang_params);
        } // if
          
        $project = $object->getProject();
        $lang_params['project_name'] = $project->getName(); 
        $lang_params['project_view_url'] = $project->getOverviewUrl();
        return lang('<a href=":user_url">:user_name</a> unlocked comments on <a href=":url">:name</a> :type in <a href=":project_view_url">:project_name</a> project', $lang_params);
      } // if
    ...
    function renderPortalHead($portal, $object = null, $in_project = false) {
    	...
    	if(instance_of($portal, 'Portal') && instance_of($object, "ProjectObject")) {
    		...
    		$rendered .= lang(':user_name unlocked comments on <a href=":object_url">:object_name</a> :object_type', array(
    			'user_name'   => $created_by->getDisplayName(true),
